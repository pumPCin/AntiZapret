...
--filter-tcp=443 --hostlist=\"%~dp0list-extended.txt\" --dpi-desync=fake,multisplit --dpi-desync-split-seqovl=681 --dpi-desync-split-pos=1 --dpi-desync-fooling=ts --dpi-desync-repeats=8 --dpi-desync-split-seqovl-pattern=\"%~dp0tls_clienthello_www_google_com.bin\" --dpi-desync-fake-tls-mod=rnd,dupsid,sni=www.google.com --new ^
...
net stop %SRVCNAME%
sc delete %SRVCNAME%
sc create %SRVCNAME% binPath= "\"%~dp0winws.exe\" %ARGS%" DisplayName= "AntiZapret Service V4 (Extended list): %SRVCNAME%" start= auto
sc description %SRVCNAME% "AntiZapret Service V4 (Extended list)"
sc start %SRVCNAME%
pause
